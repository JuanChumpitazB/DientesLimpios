...
namespace DientesLimpios.Pruebas.Aplicacion.CasosDeUso.Consultorios
{
    ...
    public class CasoDeUsoBorrarConsultorioTests
    {
        ...
        public async Task Handle_CuandoConsultorioExiste_BorrarConsultorioYPersiste()
        {
            ...
            await repositorio.Received(1).Borrar(consultorio.Id);
        ...
        [TestMethod]
        public async Task Handle_CuandoOcurreExepcion_LlamaReversarYLanzaExcepcion()
        {
            var id = Guid.NewGuid();
            var comando = new ComandoBorrarConsultorio { Id = id };
            var consultorio = new Consultorio("Consultorio A");
            ...
            repositorio.ObtenerPorId(id).Returns(consultorio);
            repositorio.Borrar(consultorio.Id).Throws(new InvalidOperationException("Error al borrar"));
