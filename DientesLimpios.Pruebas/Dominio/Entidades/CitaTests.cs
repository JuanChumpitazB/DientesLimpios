...
namespace DientesLimpios.Pruebas.Dominio.Entidades
{
    ...
    public class CitaTests
    {
        ...
        public void Constructor_FechaInicioPasado_LanzaExcepcion()
        {
            _intervalo = new IntervaloDeTiempo(DateTime.UtcNow.AddDays(-2), DateTime.UtcNow);
            var cita = new Cita(_pacienteId, _dentistaId, _consultorioId, _intervalo);

        }

        [TestMethod]
        public void Constructor_CitaProgramada_CambiaEstadoCancelada()
        {
            var cita = new Cita(_pacienteId, _dentistaId, _consultorioId, _intervalo);
            cita.Cancelar();
        ...
        [TestMethod]
        [ExpectedException(typeof(ExcepcionDeReglaDeNegocio))]
        public void Constructor_CitaNoProgramada_LanzaExcepcion()
        {
            var cita = new Cita(_pacienteId, _dentistaId, _consultorioId, _intervalo);
            cita.Cancelar();
            cita.Cancelar();
